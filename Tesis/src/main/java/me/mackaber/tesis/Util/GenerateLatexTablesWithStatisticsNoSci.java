 ...
 */
public class GenerateLatexTablesWithStatisticsNoSci implements ExperimentComponent {
  ...
  public GenerateLatexTablesWithStatisticsNoSci(Experiment<?, ?> configuration) {
  ...
  private void printData(String latexFile, int indicatorIndex, double[][][] centralTendency, double[][][] dispersion, String caption) throws IOException {
    ...
    for (int i = 0; i < experiment.getProblemList().size(); i++) {
      ...
      for (int j = 0; j < (experiment.getAlgorithmList().size() - 1); j++) {
        ...
        String m = String.format(Locale.ENGLISH, "%f", centralTendency[indicatorIndex][i][j]);
        String s = String.format(Locale.ENGLISH, "%f", dispersion[indicatorIndex][i][j]);
        os.write("$" + m + "_{" + s + "}$ & ");
      }
      ...
      String m = String.format(Locale.ENGLISH, "%f",
          centralTendency[indicatorIndex][i][experiment.getAlgorithmList().size() - 1]);
      String s = String.format(Locale.ENGLISH, "%f",
